...
bool VarDriver::readFrameCenters()
{
  ...
  while (std::getline(centerFile, line))
  {
    ...
    datetime datetime_ =
        startDate + std::chrono::hours{ hours } + std::chrono::minutes{ minutes } + std::chrono::seconds{ seconds };
    iss >> lat;
    iss >> lon;
    iss >> Vm;
    iss >> Um;
...
bool VarDriver::read_met_obs_file(int suffix, std::string& filename, std::vector<MetObs>* metData)
{
  ...
  switch (suffix)
  {
    ...
    case (ict):
      ...
      break;
    default: cout << "Unknown data type, skipping..." << endl; return false;
  }
...
bool VarDriver::read_eol(std::string& filename, std::vector<MetObs>* metObVector)
{
  ...
  while (std::getline(metFile, line))
  {
    ...
    if (line.find("Launch Site Type") == 0)
    {  // find of 0 means it's there, starting at position 0 (vs. npos)
...
/* This routine reads a 1 sec flight level data file similar to the format provided by NOAA/HRD
   Columns are:
   TIME       Lat       Lon     Head   Track      GSpd     TAS    GAlt    Press     WndDr     WndSp   Tempr    Dewpt   DVal
   PAlt     SurfP    VtWnd Pitch     Roll   Drift    Theta    Theta-e SFMRDown  SFMRSide */
...
bool VarDriver::read_sec(std::string& filename, std::vector<MetObs>* metObVector)
{
  ...
  while (std::getline(metFile, line))
  {
    ...
    datetime datetime_ =
        date + std::chrono::hours{ hours } + std::chrono::minutes{ minutes } + std::chrono::seconds{ seconds };
...
/*
  ...
  while (!in.atEnd()) {
    ...
    // Note that the longitude can sometimes be given in degrees West, which jacks up the typically negative decimal
  representation for US locations; ob.setLon(lineparts[2].toFloat()); ob.setAltitude(lineparts[7].toFloat());
...
/* GMT Time   AOA     BSP   CAS    CC     CSP    DPR     DVAL      GA     GPSA   GS      HSS     LAT      LON       PA PITCH
 * RA ROLL     SLP   SS     TA   TAS    TDA    TDD  THD   TRK    TT   V V     WD     WS Valid Flags Source Tags */
...
bool VarDriver::read_dorade(std::string& filename, std::vector<MetObs>* metObVector)
{
  ...
  std::string radardbz =
      configHash["radar_dbz"];  // If radar_dbz isn't found, this will create it -- switch to find/check? (bdobbins)
  ...
  while (std::getline(metFile, line))
  {
    ...
    while ((fabs(pmin) > 1.) and (iter < 5000))
    {
      ...
      real pprime = (refstate->getReferenceVariable(ReferenceVariable::pressref, height + 500.) -
                     refstate->getReferenceVariable(ReferenceVariable::pressref, height - 500.)) /
                    1000.;
      if (pprime != 0)
      {
...
bool VarDriver::parseXMLconfig(const XMLNode& config)
{
  ...
  {
    cout << "Unsupported array_order" << configHash["array_order"] << ". Only column-major and row-major are supported"
         << ". Aborting..." << endl;
    return false;
  }
...
{
  // default value
  Projection::ProjectionType retVal = Projection::TRANSVERSE_MERCATOR_EXACT;
  /*fixme
...
bool VarDriver::read_mesonet(std::string& filename, std::vector<MetObs>* metObVector)
{
  ...
  Nc3Var *latVar, *lonVar, *altVar, *timeVar, *tempVar, *dewpVar, *wdirVar, *wspdVar, *pressVar;
  ...
  real lat[NREC], lon[NREC], alt[NREC], obtime[NREC], temp[NREC], dewp[NREC], wdir[NREC], wspd[NREC], press[NREC];
  ...
  for (int rec = 0; rec < NREC; rec++)
  {
    ...
    datetime datetime_;
    datetime_ = std::chrono::seconds(long(obtime[rec])) +
                std::chrono::time_point<std::chrono::system_clock>{};  // bpd6 - casting issue?  What's 'rec'?
...
bool VarDriver::read_classnc(std::string& filename, std::vector<MetObs>* metObVector)
{
  ...
  Nc3Var *latVar, *lonVar, *altVar, *timeVar, *tempVar, *rhVar, *wdirVar, *wspdVar, *pressVar;
  ...
  real lat[NREC], lon[NREC], alt[NREC], obtime[NREC], temp[NREC], rh[NREC], wdir[NREC], wspd[NREC], press[NREC];
  if (!latVar->get(lat, NREC))
    return false;
  if (!lonVar->get(lon, NREC))
    return false;
...
bool VarDriver::read_aeri(std::string& filename, std::vector<MetObs>* metObVector)
{
  ...
  Nc3Var *latVar, *lonVar, *altVarBase, *altVar, *timeVarBase, *timeVar, *tempVar, *dewpVar, *pressVar;
  if (!(latVar = dataFile.get_var("lat")))
    return false;
  if (!(lonVar = dataFile.get_var("lon")))
    return false;
  ...
  real lat, lon, altBase, basetime, alt[NREC][NLVL], obtime[NREC], temp[NREC][NLVL], dewp[NREC][NLVL], press[NREC][NLVL];
  if (!latVar->get(&lat, 1))
    return false;
  if (!lonVar->get(&lon, 1))
    return false;
...
bool VarDriver::read_cfrad(std::string& fileName, std::vector<MetObs>* metObVector)
{
  ...
  {
    std::cerr << "ERROR - File '" << fileName << "' is not in a Radx supported format." << std::endl;
    return false;
  }
  ...
  for (size_t index = 0; index < rays.size(); index++)
  {
    ...
    if (ray == NULL)
    {
      std::cout << "Error: Failed to read ray " << index << " from file " << fileName << std::endl;
      return false;
    }
    ...
    if (std::isnan(radarLat) || std::isnan(radarLon) || std::isnan(radarAlt))
    {
    ...
    for (size_t gateIndex = 0; gateIndex < nGates - stride; gateIndex += stride)
    {
      ...
      if ((vr != -999.0) || (dz != -999.0))
      {
        ...
        // Take into account curvature of the earth for the height of the radar beam
        real relZ = sqrt(range * range + rEarth * rEarth + 2.0 * range * rEarth * sin(el * Pi / 180.0)) - rEarth;
