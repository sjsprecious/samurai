...
const real BSplineBase<T>::BoundaryConditions[9][4] = {
  //	0	1	M-1	M
  { -4, -1, -1, -4 }, { 0, 1, 1, 0 },  { 2, -1, -1, 2 },  { -4, -1, 1, 0 }, { -4, -1, -1, 2 },
  { 0, 1, -1, -4 },   { 0, 1, -1, 2 }, { 2, -1, -1, -4 }, { 2, -1, 1, 0 }
};
...
BSplineBase<T>::BSplineBase(const BSplineBase<T> &bb)
    : K(bb.K),
      BC(bb.BC),
      OK(bb.OK),
      base(new BSplineBaseP<T>(*bb.base))
{
  xmin = bb.xmin;
  xmax = bb.xmax;
  alpha = bb.alpha;
...
template<class T>
BSplineBase<T>::BSplineBase(const T *x, int nx, real wl, int bc, int num_nodes)
    : K(3),
      OK(false),
      base(new BSplineBaseP<T>)
{
...
template<class T>
bool BSplineBase<T>::setDomain(const T *x, int nx, real wl, int bc, int num_nodes)
{
  ...
  if (Setup(num_nodes))
  {
    if (Debug())
    {
      std::cerr << "Using M node intervals: " << M << " of length DX: " << DX << std::endl;
      std::cerr << "X min: " << xmin << " ; X max: " << xmax << std::endl;
      std::cerr << "Data points per interval: " << (float)NX / (float)M << std::endl;
      std::cerr << "Nodes per wavelength: " << (float)waveLength / (float)DX << std::endl;
...
template<class T>
bool BSplineBase<T>::setDomainGQ(const T *x, int nx, real wl, int bc, int num_nodes)
{
  if (nx <= 0 || x == 0 || wl < 0 || bc < 0 || bc > 8)
  {
    return false;
 ...
 */
template<class T>
real BSplineBase<T>::Alpha(real wl)
{
...
template<class T>
real BSplineBase<T>::AlphaGQ(real wl)
{
  // K is the degree of the derivative constraint: 1, 2, or 3
  real a = (real)(wl / (2 * PI * DX));
  a *= a;  // a^2
...
template<class T>
inline real BSplineBase<T>::Beta(/* unsigned */ int m)  // TODO called with int
{
  if (m > 1 && m < M - 1)
    return 0.0;
  if (m >= M - 1)
 ...
 */
template<class T>
real BSplineBase<T>::Basis(int m, T x)
{
  real y = 0;
  real xm = xmin + (m * DX);
 ...
 */
template<class T>
real BSplineBase<T>::DBasis(int m, T x)
{
  real dy = 0;
  real xm = xmin + (m * DX);
  real delta = (real)(x - xm) * (real)DXrecip;
...
template<class T>
real BSplineBase<T>::qDelta(int m1, int m2)
/*
 ...
 */
{
  ...
  static const real qparts[3][4][4] = { { { 0.11250, 0.63750, 0.63750, 0.11250 },
...
template<class T>
inline real BSplineBase<T>::Ratiod(int &ni, real &deltax, real &ratiof)
{
  deltax = (xmax - xmin) / ni;
...
template<class T>
bool BSplineBase<T>::SetupGQ(int num_nodes)
{
  std::vector<T> &X = base->X;
 ...
 */
template<class T>
BSpline<T>::BSpline(const T *x, int nx, const T *y, real wl, int bc_type, int num_nodes)
...
bool BSpline<T>::solve(const T *y)
{
  ...
  else
  {
    ...
    if (Debug() && M < 30)
    {
      ...
      std::cerr << "LU factor of (P+Q) = " << std::endl << base->Q << std::endl;
    }
  }
  return (OK);
}
 ...
 */
template<class T>
bool BSpline<T>::solveGQ(const T *y)
{
  // if (! OK)
  //	return false;
 ...
 */
template<class T>
bool BSpline<T>::solveBGQ(const T *b)
{
  // if (! OK)
  //	return false;
 ...
 */
template<class T>
const T *BSpline<T>::solveInverseGQ(const T *b)
{
  if (!OK)
    return NULL;
...
{
  if (dumpMatrixToArray(base->Q, base->q))
    return base->q;
  return 0;
}
