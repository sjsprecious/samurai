...
void RecursiveFilter::getIsotropicFilterCoefficients()
{
  ...
  K[6] = b[1][3] * sigma + b[2][3] * (pow(sigma, 2)) / factorial(2) + b[3][3] * (pow(sigma, 3)) / factorial(3);
  K[8] = b[1][4] * sigma + b[2][4] * (pow(sigma, 2)) / factorial(2) + b[3][4] * (pow(sigma, 3)) / factorial(3) +
         b[4][4] * (pow(sigma, 4)) / factorial(4);
  ...
  std::cout << "4th Order Recursive Filter coefficients computed for lengthscale " << lengthScale << " Delta X" << std::endl;
...
bool RecursiveFilter::filterArray(double* array, const int& arrLength)
{
  ...
  {
    q[i] = beta * p[i] + alpha[1] * q[i - 1] + alpha[2] * q[i - 2] + alpha[3] * q[i - 3] + alpha[4] * q[i - 4];
  }
  ...
  {
    s[i] = beta * q[i] + alpha[1] * s[i + 1] + alpha[2] * s[i + 2] + alpha[3] * s[i + 3] + alpha[4] * s[i + 4];
...
void RecursiveFilter::getAnisotropicFilterCoefficients(const double* tau, const int& arr)
{
  ...
  coeff[3] = b[1][3] * sigma + b[2][3] * (pow(sigma, 2)) / factorial(2) + b[3][3] * (pow(sigma, 3)) / factorial(3);
  coeff[4] = b[1][4] * sigma + b[2][4] * (pow(sigma, 2)) / factorial(2) + b[3][4] * (pow(sigma, 3)) / factorial(3) +
             b[4][4] * (pow(sigma, 4)) / factorial(4);
  ...
  for (int i = 0; i < arrLength; i++)
  {
    ...
    {
      K1[i][i - 1] =
          -(sigma * (tau[i] - tau[i - 1])) / sqrt(sigma * (htau[i] - htau[i - 1]) * sigma * (htau[i - 1] - htau[i - 2]));
    }
    else if (i == 1)
    {
      K1[i][i - 1] =
          -(sigma * (tau[i] - tau[i - 1])) / sqrt(sigma * (htau[i] - htau[i - 1]) * sigma * (htau[i - 1] + htau[i - 1]));
    }
    if ((i > 0) and (i < (arrLength - 1)))
    ...
    {
      K1[i][i + 1] =
          -(sigma * (tau[i + 1] - tau[i])) / sqrt(sigma * (htau[i] - htau[i - 1]) * sigma * (htau[i + 1] - htau[i]));
    }
    else
    {
      K1[i][i + 1] =
          -(sigma * (tau[i] - tau[i - 1])) / sqrt(sigma * (htau[i] - htau[i - 1]) * sigma * (htau[i] - htau[i - 1]));
    }
    if (i == 0)
    {
  ...
  aalpha[3][0] = 0;
  aalpha[4][0] = 0;
  std::cout << "0\t" << abeta[0] << "\t" << aalpha[1][0] << "\t" << aalpha[2][0] << "\t" << aalpha[3][0] << "\t"
            << aalpha[4][0] << "\t(" << abeta[0] + aalpha[1][0] + aalpha[2][0] + aalpha[3][0] + aalpha[4][0] << ")\n";
  ...
  aalpha[3][1] = 0;
  aalpha[4][1] = 0;
  std::cout << "1\t" << abeta[1] << "\t" << aalpha[1][1] << "\t" << aalpha[2][1] << "\t" << aalpha[3][1] << "\t"
            << aalpha[4][1] << "\t(" << abeta[1] + aalpha[1][1] + aalpha[2][1] + aalpha[3][1] + aalpha[4][1] << ")\n";
  ...
  aalpha[3][2] = 0;
  aalpha[4][2] = 0;
  std::cout << "2\t" << abeta[2] << "\t" << aalpha[1][2] << "\t" << aalpha[2][2] << "\t" << aalpha[3][2] << "\t"
            << aalpha[4][2] << "\t(" << abeta[2] + aalpha[1][2] + aalpha[2][2] + aalpha[3][2] + aalpha[4][2] << ")\n";
  ...
  aalpha[4][3] = 0;
  std::cout << "3\t" << abeta[3] << "\t" << aalpha[1][3] << "\t" << aalpha[2][3] << "\t" << aalpha[3][3] << "\t"
            << aalpha[4][3] << "\t(" << abeta[3] + aalpha[1][3] + aalpha[2][3] + aalpha[3][3] + aalpha[4][3] << ")\n";
  ...
  for (int i = 4; i < arrLength; i++)
  {
    ...
    std::cout << i << "\t" << abeta[i] << "\t" << aalpha[1][i] << "\t" << aalpha[2][i] << "\t" << aalpha[3][i] << "\t"
              << aalpha[4][i] << "\t(" << sum << ")\n";
  }
...
bool RecursiveFilter::aniFilterArray(double* array, const int& arrLength)
{
  ...
  for (int i = order; i <= maxi; i++)
  {
    q[i] = abeta[i] * p[i] + aalpha[1][i] * q[i - 1] + aalpha[2][i] * q[i - 2] + aalpha[3][i] * q[i - 3] +
           aalpha[4][i] * q[i - 4];
  }
  ...
  for (int i = maxi - order; i >= 0; i--)
  {
    s[i] = abeta[i] * q[i] + aalpha[1][i] * s[i + 1] + aalpha[2][i] * s[i + 2] + aalpha[3][i] * s[i + 3] +
           aalpha[4][i] * s[i + 4];
    // std::cout << s[i] << std::endl;
  }
  delete[] A;
