...
bool Dorade::readSwpfile()
{
  ...
  sweepread(filename.data(), ssptr, vptr, rptr, cptr, cfptr, pptr, sptr, ryptr, aptr, dptr);
  return true;
}
...
bool Dorade::writeSwpfile()
{
  ...
  int flag = 0;
  sweepwrite(filename.data(), ssptr, vptr, rptr, cptr, cfptr, pptr, sptr, ryptr, aptr, dptr, flag);
...
datetime Dorade::getRayTime(int &ray)
{
  ...
  auto result = sys_days(ymd) + days(ryptr[ray].julian_day - 1) + hours(ryptr[ray].hour) + minutes(ryptr[ray].min) +
                seconds(ryptr[ray].sec);

  return result;
}

bool Dorade::copyField(
    const std::string &oldFieldName,
    const std::string &newFieldName,
    const std::string &newFieldDesc,
    const std::string &newFieldUnits)
{
...
/**************************************************/
void Dorade::sweepread(
    const char *swp_fname,
    ...
    int doradeFlag)
{
  ...
  for (int i = 0; i < (sptr->num_rays); i++)
  {
    ...
    for (int j = 0; j < (rptr->num_param_desc); j++)
    {
      ...
      num_words =
          dd_compress((unsigned short *)&arr_uncom, (unsigned short *)&arr_com, (unsigned short)baddata_flag, arrsize);
...
void Dorade::read_celv(FILE *fp, struct celv_info *cptr, int desc_len)
{
  ...
  /* GATE SPACING */
  if ((fread(cptr->gate_spacing, sizeof(float), cptr->total_gates, fp)) != (unsigned int)cptr->total_gates)
  {
    ...
    struct rdat_info *rdat)
{
  ...
  if ((fread(tempname, sizeof(char), PARM_NAME_LEN, fp)) != PARM_NAME_LEN)
  {
  ...
  *match = true;
  ...
  /* SIZE OF ARRAY */
  arrsize = (desc_len - (IDENT_LEN + sizeof(int) + PARM_NAME_LEN)) / datasize;
  /* READ IN THE DATA */
  if (datasize == 1)
  {
  ...
  {
    read_sh_arr(fp, arrsize, beam_count, total_gates, compression, baddata_flag, scale_fac, rdat);
  }
  else if (datasize == 3)
  {
    ...
    struct rdat_info *rdat)
{
  ...
  if (compression == true)
  {
    ...
    num = dd_hrd16_uncompressx(arr_com, arr_uncom, baddata_flag, &empty_run, total_gates, beam_count);
    if (num > total_gates)
    {
...
int Dorade::dd_hrd16_uncompressx(short *ss, short *dd, int flag, int *empty_run, int wmax, int beam_count)
...
# wmax indicate the maximum number of 16-bit words the routine can
  ...
  while (*ss != 1)
  {                   /* 1 is an end of compression flag */
    ...
    if (*ss & 0x8000)
    { /* high order bit set implies data! */
      *empty_run = 0;
      ss++;
      for (; n--;)
      {
...
int Dorade::dd_compress(unsigned short *src, unsigned short *dst, unsigned short flag, int n)
{
   ...
   */
  if (data_run)
  { /* just stuff it no matter what it is */
    if (ss == end)
    {
      *dd++ = *ss;
      kount++;
  ...
  { /* there is one last datum at the end of a flag run */
    if (kount == 2)
    { /* special case for just two flags */
      *rlcode = SIGN16 | 3;
      *dd++ = flag;
      *dd++ = flag;
      *dd++ = *ss;
...
void Dorade::calcAirborneAngles(
    struct asib_info *asib,
    struct cfac_info *cfac,
    struct ryib_info *ra,
    struct radar_angles *angles)
{
  ...
  angles->x = xsubt = (cos_theta_rc * sinD * cos_tau_a * sinP + cosD * sin_theta_rc * cos_tau_a - sinD * cosP * sin_tau_a);
  angles->y = ysubt = (-cos_theta_rc * cosD * cos_tau_a * sinP + sinD * sin_theta_rc * cos_tau_a + cosP * cosD * sin_tau_a);
  angles->z = zsubt = (cosP * cos_tau_a * cos_theta_rc + sinP * sin_tau_a);
