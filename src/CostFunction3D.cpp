...
void CostFunction3D::initialize(HashMap* config, real* bgU, real* obs, ReferenceState* ref)
{
...
void CostFunction3D::initState(const int iteration)
{
  ...
  if (((*configHash)["output_mish"] == "true") or ((*configHash)["save_mish"] == "true"))
  {
    mishFlag = 1;
  }
  else
...
#pragma acc data copyin(stateA [0:nState]) copyout(bgState [0:nState])
  ...
  for (int var = 0; var < varDim; var++)
  {
    ...
    if (varScale)
    {
      ...
      cout << "Variable " << var << " RMS = " << varScale << "\t BG Error = " << bgError[var] << " ( " << errPct << " %)"
           << endl;
    }
    else
    {
      cout << "Variable " << var << " RMS = " << varScale << "\t BG Error = " << bgError[var] << " ( Infinite! %)" << endl;
    }
  }
...
void CostFunction3D::SBtransform(const real* Ustate, real* Bstate)
{
  ...
  for (var = 0; var < varDim; var++) {
    cout << " min(rankHash[iBCL[var]],1): " <<  min(rankHash[iBCL[var]],1) << " max(iDim-1-rankHash[iBCR[var]],iDim-2): " <<
max(iDim-1-rankHash[iBCR[var]],iDim-2) << "\n"; is = min(rankHash[iBCL[var]],1); ie = max(iDim-1-rankHash[iBCR[var]],iDim-2);
        ...
        iis=max(0,ii-1);iie=min(ii+2,iDim);
              ...
              jjs=max(0,jj-1);jje=min(jj+2,jDim);
                                for (jNode = jjs; jNode <= jje; ++jNode) {
                                                                jbasis = Basis(jNode, j, jDim-1, jMin, DJ, DJrecip, 0,
jBCL[var], jBCR[var]); uJ = jIndex*2 + (jmu+1)/2;
                    ...
                    kks=max(0,kk-1);kke=min(kk+2,kDim);
                      ...
                      ui = INDEX(uI, uJ, kIndex*2 + (kmu+1)/2, (iDim-1)*2, (jDim-1)*2, varDim, var);
                                                                if (Ustate[ui] == 0) continue;
                                                                        kbasis = Basis(kNode, k, kDim-1, kMin, DK, DKrecip,
0, kBCL[var], kBCR[var]); bi = INDEX(iNode, jNode, kNode, iDim, jDim, varDim, var); Bstate[bi] += Ustate[ui] * 0.125 * ibasis
* jbasis * kbasis;
                                                                }
                                                                }
                                                                }
          ...
          for (int jIndex = min(rankHash[jBCL[var]], 1); jIndex < max(jDim - 1 - rankHash[jBCR[var]], jDim - 2); jIndex++)
          {
            for (int jmu = -1; jmu <= 1; jmu += 2)
            {
              ...
              for (int jjNode = (jj - 1); jjNode <= (jj + 2); ++jjNode)
              {
                ...
                for (int kIndex = min(rankHash[kBCL[var]], 1); kIndex < max(kDim - 1 - rankHash[kBCR[var]], kDim - 2);
                     kIndex++)
                {
...
#pragma acc loop vector
            ...
            if (kFilterScale > 0)
              kFilter->filterArray(kTemp, kq, ks, kDim);
#pragma acc loop vector
            for (int kIndex = 0; kIndex < kDim; kIndex++)
            {
...
#pragma acc loop vector
            ...
            if (jFilterScale > 0)
              jFilter->filterArray(jTemp, jq, js, jDim);
#pragma acc loop vector
            for (int jIndex = 0; jIndex < jDim; jIndex++)
            {
              index = INDEX(iIndex, jIndex, kIndex, iDim, jDim, varDim, var);
...
#pragma acc loop vector
            ...
            if (iFilterScale > 0)
              iFilter->filterArray(iTemp, iq, is, iDim);
#pragma acc loop vector
            for (int iIndex = 0; iIndex < iDim; iIndex++)
            {
              // D
...
void CostFunction3D::obAdjustments()
{
  ...
  for (int m = 0; m < mObs; m++)
  {
    ...
    if ((i < iMin) or (i > iMax) or (j < jMin) or (j > jMax) or (k < kMin) or (k > kMax))
    {
    ...
    if ((type == MetObs::radar) or (type == MetObs::qscat) or (type == MetObs::ascat) or (type == MetObs::AMV) or
        (type == MetObs::lidar))
    {
      obsVector[mi] *= rhoBG;
    }
  ...
  else if (m == 2)
  {
    ...
    switch (BL)
    {
      ...
      case R1T10:
        ...
        break;
      case R2T10: node = 0; coeffmod = 1.0;
      case R2T20: node = 0; coeffmod = -1.0;
      case R3: return 0;
      case PERIODIC:
        node = M;
        coeffmod = 1.;
...
void CostFunction3D::calcSplineCoefficients(
    const int& Dim,
    const real& eq,
    const int* BCL,
    const int* BCR,
  ...
  for (int var = 0; var < varDim; var++)
  {
    ...
    switch (BCL[var])
    {
      ...
      case R2T10:
        ...
        G[0][1] = -0.5;
        break;
      case R2T20: G[0][0] = -1.0; break;
      case PERIODIC: G[mDim - 1][0] = 1; break;
      default: break;
    }
    switch (BCR[var])
    {
      ...
      case R1T0:
        ...
        G[mDim - 2][pDim - 1] = -1.0;
        break;
      case R1T1: G[mDim - 2][pDim - 1] = 1.0; break;
      case R1T2:
        G[mDim - 1][pDim - 1] = 2.0;
        G[mDim - 2][pDim - 1] = -1.0;
        break;
      case R2T10:
        ...
        G[mDim - 1][pDim - 2] = -0.5;
        break;
      case R2T20: G[mDim - 1][pDim - 1] = -1.0; break;
      case PERIODIC:
        G[0][pDim - 2] = 1;
        G[1][pDim - 1] = 1;
        break;
...
#pragma acc enter data copyin(H[:nnz])
  std::cout << "Memory usage for [obsVector]     (Mbytes): "
            << sizeof(real) * (mObs * (7 + varDim * derivDim)) / (1024.0 * 1024.0) << std::endl;
  std::cout << "Memory usage for [obsData]       (Mbytes): " << sizeof(real) * (mObs) / (1024.0 * 1024.0) << std::endl;
  std::cout << "Memory usage for [HCq]           (Mbytes): "
            << sizeof(real) * (mObs + (iDim * jDim * kDim)) / (1024.0 * 1024.0) << std::endl;
  std::cout << "Memory usage for [IH,JH]         (Mbytes): "
            << (sizeof(uint64_t) * (mObs + 1) + sizeof(int32_t) * nnz) / (1024. * 1024.) << std::endl;
  std::cout << "Memory usage for [state]         (Mbytes): " << sizeof(real) * (nState) / (1024. * 1024.) << std::endl;
#pragma acc enter data copyin(Ht[:nnz])
  std::cout << "Memory usage for [IHt,JHt]         (Mbytes): "
            << (sizeof(uint64_t) * (nState + 1) + sizeof(int32_t) * nnz) / (1024. * 1024.) << std::endl;
...
bool CostFunction3D::copyResults(int iDim, int jDim, int kDim, float* u, float* v, float* w, float* t, float* p)
{
