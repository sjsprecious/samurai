...
void MetObs::setRH(const float& RH)
{
  ...
  {
    float t = temperature;
    float es = (E_3 * exp(_A_ * log(T_3 / t)) * exp((_A_ + _B_) * (1 - T_3 / t)));
    float e = es * RH / 100;
    float u = log(e / E_3);
...
float MetObs::getPolarUwind(const float& centerLat, const float& centerLon) const
{
  if (windSpeed != -999)
  {
    ...
    float fac_lat = 111.13209 - 0.56605 * cos(2.0 * centerLatRadians) + 0.00012 * cos(4.0 * centerLatRadians) -
                    0.000002 * cos(6.0 * centerLatRadians);
    float fac_lon =
        111.41513 * cos(centerLatRadians) - 0.09455 * cos(3.0 * centerLatRadians) + 0.00012 * cos(5.0 * centerLatRadians);
...
{
  float t = temperature;
  float e = (E_3 * exp(_A_ * log(T_3 / t)) * exp((_A_ + _B_) * (1 - T_3 / t)));
...
{
  if ((this->time == other.time) && (this->stationName == other.stationName))
    return true;
  return false;
}
