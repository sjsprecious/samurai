...
datetime ParseDate(const char *string, const char *fmt)
{
  ...
  {
    std::cout << "ERROR PARSING TIME; quitting."
              << " ( string = " << string << ", fmt = " << fmt << " ) " << std::endl;
    exit(1);
  }
  ...
  // std::cout << "DEBUG: tt = " << tt << " put_time : " << std::put_time(&tm, "%c %Z") << " gmtime : " << std::gmtime(&tt)
  // << std::endl;
...
datetime ParseTime(const char *string, const char *fmt)
{
  ...
  {
    std::cout << "ERROR PARSING TIME; quitting."
              << " ( epochstring = " << epochstring << ", epochfmt = " << epochfmt << " ) " << std::endl;
    exit(1);
  }
  std::time_t tt = timegm(&tm);
  strftime(buffer, 512, "%c %Z", &tm);
