...
class Dorade
{
 ...
 private:
  ...
  double FMOD360(double x)
  {
    return (fmod((double)((x) + 720.), (double)360.));
  };
  ...
  int dd_hrd16_uncompressx(short *ss, short *dd, int flag, int *empty_run, int wmax, int beam_count);
  int dd_compress(unsigned short *src, unsigned short *dst, unsigned short flag, int n);
  void calcAirborneAngles(struct asib_info *asib, struct cfac_info *cfac, struct ryib_info *ra, struct radar_angles *angles);
};
