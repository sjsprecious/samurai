...
bool CostFunctionXYZ::SItransform(size_t numVars, double *finalAnalysis, double *mishData, real *Astate, ofstream *outStream)
{
  ...
  {  // SItransform loops on both mish and mesh datapoints
                ...
                for (int khalf = 0; khalf <= mishFlag; khalf++)
                {
                  for (int kmu = -khalf; kmu <= khalf; kmu++)
                  {
                          ...
                          for (int jjNode = jj - 1; jjNode <= jj + 2; ++jjNode)
                          {
                            ...
                            switch (var)
                            {
                              ...
                              case 5:
                                ...
                                break;
                              case 6: qrprime += Astate[aIndex + 6] * basis3x; break;
                            }
                          }
                        }
                      }
                    ...
                    {  // We are on the Mish
                      ...
                      int64_t uIndex =
                          varDim * (iDim - 1) * 2 * (jDim - 1) * 2 * uK + varDim * (iDim - 1) * 2 * uJ + varDim * uI;
                    ...
                    if (((*configHash)["output_mish"] == "false") and (ihalf or jhalf or khalf))
                      continue;  // halfway point on the Mesh
                    ...
                    {
                      if (heightm > 30000)
                        std::cout << "==== u: " << u << ", rhoa: " << rhoa << ", rhou: " << rhou << ", rhoq: " << rhoq
                                  << ", rho: " << rho << ", qv: " << qv << ", rhoBar: " << rhoBar
                                  << ", rhoprime: " << rhoprime << ", heightm: " << heightm << std::endl;
                    ...
                    real satvp =
                        exp(-6096.9385 / temp + 16.635794 - 2.711193e-2 * temp + 1.673952e-5 * temp * temp +
                            2.433502 * log(temp));
                    ...
                    pdx = (tdx * rhoa + rhoadx * temp) * 287. / 100. +
                          (tdx * rhoq + (rhoadx * qv + qvdx * rhoa) * temp / 1000.0) * 461. / 100.;
                    pdy = (tdy * rhoa + rhoady * temp) * 287. / 100. +
                          (tdy * rhoq + (rhoady * qv + qvdy * rhoa) * temp / 1000.0) * 461. / 100.;
                    pdz = (tdz * rhoa + rhoadz * temp) * 287. / 100. +
                          (tdz * rhoq + (rhoadz * qv + qvdz * rhoa) * temp / 1000.0) * 461. / 100.;
                    ...
                    if (refmask != "None")
                    {
                      ...
                      if (!terrainFile.is_open())
                      {
                        // comment out the error message  cout << "No terrain file to read in CostFunctionXYZ.cpp ..." <<
                        // endl;
                        terrain_index = 0;
                      }
                      else
                    ...
                    if (outStream != NULL)
                    {  // TODO number of vars...
                      *outStream << scientific << i << "\t" << j << "\t" << k << "\t" << u << "\t" << v << "\t" << w << "\t"
                                 << vorticity << "\t" << divergence << "\t" << qv << "\t" << rho << "\t" << temp << "\t"
                                 << press << "\t" << theta << "\t" << thetae << "\t" << thetaes << "\t" << udx << "\t" << udy
                                 << "\t" << udz << "\t" << vdx << "\t" << vdy << "\t" << vdz << "\t" << wdx << "\t" << wdy
                                 << "\t" << wdz << "\t" << rhowdz * 100. << "\t" << mcresidual << "\t" << qr << "\n";
                    }
...
bool CostFunctionXYZ::writeNetCDF(const std::string &netcdfFileName)
{
  ...
  if (!(u = dataFile.add_var("U", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(v = dataFile.add_var("V", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(w = dataFile.add_var("W", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(wspd = dataFile.add_var("WSPD", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(relhum = dataFile.add_var("RH", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(hprime = dataFile.add_var("HP", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(qvprime = dataFile.add_var("QVP", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(rhoprime = dataFile.add_var("RHOAP", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(tprime = dataFile.add_var("TP", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(pprime = dataFile.add_var("PP", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(vorticity = dataFile.add_var("VORT", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(divergence = dataFile.add_var("DIV", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(okuboweiss = dataFile.add_var("OW", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(strain = dataFile.add_var("STRAIN", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(tpw = dataFile.add_var("TPW", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(rhou = dataFile.add_var("RHOU", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(rhov = dataFile.add_var("RHOV", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(rhow = dataFile.add_var("RHOW", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(rho = dataFile.add_var("RHOA", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(press = dataFile.add_var("P", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(temp = dataFile.add_var("T", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(qv = dataFile.add_var("QV", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(h = dataFile.add_var("H", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if ((*configHash)["qr_variable"] == "dbz")
  {
    if (!(qr = dataFile.add_var("DBZ", nc3Float, timeDim, lvlDim, latDim, lonDim)))
      return NC_ERR;
  }
  else
  {
    if (!(qr = dataFile.add_var("QR", nc3Float, timeDim, lvlDim, latDim, lonDim)))
      return NC_ERR;
  }
  if (!(absVorticity = dataFile.add_var("ABSVORT", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(dewp = dataFile.add_var("DEWPOINT", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(theta = dataFile.add_var("THETA", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(thetae = dataFile.add_var("THETAE", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(thetaes = dataFile.add_var("THETAES", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(dudx = dataFile.add_var("DUDX", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(dvdx = dataFile.add_var("DVDX", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(dwdx = dataFile.add_var("DWDX", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(dudy = dataFile.add_var("DUDY", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(dvdy = dataFile.add_var("DVDY", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(dwdy = dataFile.add_var("DWDY", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(dudz = dataFile.add_var("DUDZ", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(dvdz = dataFile.add_var("DVDZ", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(dwdz = dataFile.add_var("DWDZ", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(dtdx = dataFile.add_var("DTDX", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(dqdx = dataFile.add_var("DQVDX", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(dpdx = dataFile.add_var("DPDX", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(dtdy = dataFile.add_var("DTDY", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(dqdy = dataFile.add_var("DQVDY", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(dpdy = dataFile.add_var("DPDY", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(dtdz = dataFile.add_var("DTDZ", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(dqdz = dataFile.add_var("DQVDZ", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(dpdz = dataFile.add_var("DPDZ", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(drhodx = dataFile.add_var("DRHODX", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(drhody = dataFile.add_var("DRHODY", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(drhodz = dataFile.add_var("DRHODZ", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(mcresidual = dataFile.add_var("MCRESIDUAL", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  ...
  if (fractl_mode)
  {
    if (!(U_std = dataFile.add_var("Final_U_std", nc3Float, timeDim, lvlDim, latDim, lonDim)))
      return NC_ERR;
    if (!(V_std = dataFile.add_var("Final_V_std", nc3Float, timeDim, lvlDim, latDim, lonDim)))
      return NC_ERR;
    if (!(W_std = dataFile.add_var("Final_W_std", nc3Float, timeDim, lvlDim, latDim, lonDim)))
      return NC_ERR;
  }
...
bool CostFunctionXYZ::writeAsi(const std::string &asiFileName)
{
  ...
  /*fixme
    ...
    fieldNames  << "U" << "V" << "W" << "WS" << "RH"<< "HP" << "QP" << "RP" << "TP" << "PP" << "VO" << "DV" << "OW" << "S" <<
    "PW"
                << "MU" << "MV" << "MW" << "RO" << "PS" << "TK" << "QV" << "HH" << "DZ" << "AV" << "DP" << "TH" << "TE" <<
    "TS"; id[175] = fieldNames.size(); for(int n = 0; n < id[175]; n++) { QString name_1 = fieldNames.at(n).left(1); QString
    name_2 = fieldNames.at(n).mid(1,1); int int_1 = *name_1.toLatin1().data(); int int_2 = *name_2.toLatin1().data(); id[176
    + (5 * n)] = (int_1 * 256) + int_2; id[177 + (5 * n)] = 8224; id[178 + (5 * n)] = 8224; id[179 + (5 * n)] = 8224; id[180
    + (5 * n)] = 1;
    }
