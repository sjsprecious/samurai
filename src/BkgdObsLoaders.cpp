...
BkgdObsLoader *BkgdObsLoaderFactory::createBkgdObsLoader(BkgdObsLoader::bg_loader_t t)
{
  switch (t)
  {
    case BkgdObsLoader::BG_LOADER_PRE: return new BkgdObsPreLoader();
    case BkgdObsLoader::BG_LOADER_SPLINE:
      return new BkgdObsSplineLoader();  // Default should be: BkgdObsSplineLoader, If I want to load WRF obs, I need to
                                         // change it to BkgdObsWRFLoader
    case BkgdObsLoader::BG_LOADER_KD: return new BkgdObsKDLoader();
...
bool BkgdObsLoader::timeCheck(real time, datetime &startTime, datetime &endTime, int &tci)
{
  ...
  if ((bgTime < startTime) or (bgTime > endTime))
  {
    ...
    std::cout << std::endl << "time: " << bgTimestring << ", start: " << tcstart << ", end: " << tcend << std::endl;
    exit(1);
    return false;
  }
...
KD_tree *BkgdObsLoader::buildKDTree(std::vector<double> &bgIn)
{
  ...
  for (long i = 0; i < numPoints; i++)
  {
    ...
    if (debug)
      std::cout << "bgX: " << matrix[i][0] << ", bgY: " << matrix[i][1] << ", bgZ: " << matrix[i][2] << std::endl;
  ...
  return new KD_tree((const KD_real **)matrix, numPoints, numDims);
}

void BkgdObsLoader::dumpBgIn(int from, int to, std::vector<real> &bgIn)
{
  std::cout << std::endl
            << "---------------------- bgIn[" << from << ":" << to << "] ----------------" << std::endl
            << std::endl;
  for (int i = from; i < to; i++)
  {
    int index = i * 11;
    std::cout << i << ": x: " << bgIn[index] << ", y: " << bgIn[index + 1] << ", z: " << bgIn[index + 2]
              << ", time: " << bgIn[index + 3] << ", rhou: " << bgIn[index + 4] << ", rhov: " << bgIn[index + 5]
              << ", rhow: " << bgIn[index + 6] << std::endl;
  }
}
...
void BkgdObsLoader::dumpBgU(int from, int to, real *bgu)
{
  ...
  for (int i = from; i < to; i++)
  {
    ...
    std::cout << index << ": U: " << bgu[index] << ", V: " << bgu[index + 1] << ", W: " << bgu[index + 2]
              << ", tprime: " << bgu[index + 3] << ", qvPrime: " << bgu[index + 4] << ", rhoPrime: " << bgu[index + 5]
              << ", qr: " << bgu[index + 6] << std::endl;
  }
...
void BkgdObsLoader::bgu2nc(const char *fname, real *bgu)
{
  ...
  if (data == NULL)
  {
    std::cout << "BkgdObsLoader::dumpBguAsNCDF: Failed to allocate data array" << std::endl;
    return;
  }
          ...
          for (int jmu = -1; jmu <= 1; jmu += 2)
          {
            for (int ki = -1; ki < (kdim); ki++)
            {
              for (int kmu = -1; kmu <= 1; kmu += 2)
              {
                ...
                int bIndex =
                    numVars * (idim + 1) * 2 * (jdim + 1) * 2 * bgK + numVars * (idim + 1) * 2 * bgJ + numVars * bgI;
...
bool BkgdObsSplineLoader::initialize(
    HashMap *config,
    std::vector<FrameCenter> frames,
    BkgdAdapter *adapter,
    Projection proj,
...
bool BkgdObsSplineLoader::loadBkgdObs(std::vector<real> &bgIn)
{
  ...
  std::cout << "Loading background onto Gaussian mish with " << iROI << " grid length radius of influence in i direction"
            << std::endl;
  ...
  std::cout << "iROI: " << iROI << ", jROI: " << jROI << ", maxGridDist: " << maxGridDist << std::endl;
  ...
  while (bkgdAdapter->next(time, lat, lon, alt, u, v, w, t, qv, rhoa, qr, terrain_hgt))
  {
    ...
    if (runMode == RUN_MODE_XYZ)
    {
      if ((bgX < (imin - iincr - (iROI * iincr * maxGridDist))) or (bgX > (imax + iincr + (iROI * iincr * maxGridDist))) or
          (bgY < (jmin - jincr - (jROI * jincr * maxGridDist))) or (bgY > (jmax + jincr + (jROI * jincr * maxGridDist))) or
          (bgZ < kmin))
      {  // Allow for higher values for interpolation purposes
        domainProblem++;
    ...
    else if (runMode == RUN_MODE_RTZ)
    {
      if ((bgRadius < (imin - iincr - (iROI * iincr * maxGridDist))) or
          ...
          (bgTheta > jmax + jincr + (jROI * jincr * maxGridDist)) or (bgZ < kmin))
      {  // Exceeding the Theta domain only makes sense for sectors
        domainProblem++;
        continue;
      }
    ...
    if ((logheights.size() != 0) && (logZ <= logheights.back()))
    {
      ...
      if (logheights.size() == 1)
      {
        std::cerr << "Error at " << lat << ", " << lon << ", " << bgZ << std::endl
                  ...
                  << "Please check Background.in to ensure sorting by descending Z coordinate and re-run." << std::endl;
        levelProblem++;
        return false;
      }
  ...
  std::cout << "timeProblem: " << timeProblem << ", domainProblem: " << domainProblem << ", radiusProblem: " << radiusProblem
            << ", levelProblem: " << levelProblem << ", splineProblem: " << splineProblem << std::endl;
              ...
              for (int jmu = -1; jmu <= 1; jmu += 2)
              {
                ...
                for (unsigned int var = 0; var < numVars; var++)
                {
                  ...
                  else
                  {
                    ...
                    if (emptybg.size() < 15)
                    {
                      std::cout << "Empty background mish for variable " << var << " at " << iPos << ", " << jPos << ", "
                                << kPos << std::endl;
                    }
...
#pragma omp parallel for
  ...
  for (int ki = -1; ki < kdim; ki++)
  {
    SplineD *bgSpline = new SplineD(&logheights.front(), logheights.size(), uBG.data(), waveLen, SplineBase::BC_ZERO_SECOND);
    if (!bgSpline->ok())
    {
    ...
    for (int kmu = -1; kmu <= 1; kmu += 2)
    {
      ...
      if (logzPos < logheights[0])
        logzPos = logheights[0];
...
bool BkgdObsKDLoader::overwriteBgu(const char *fname)
{
  ...
  while (ifs >> idx >> dummy >> dummy >> u >> dummy >> dummy >> v >> dummy >> dummy >> w >> dummy >> dummy >> tp >> dummy >>
         dummy >> qvp >> dummy >> dummy >> rp >> dummy >> dummy >> qr)
  {
    ifs.ignore(100, '\n');
...
bool BkgdObsKDLoader::loadBkgdObs(std::vector<real> &bgIn)
{
  ...
  std::cout << "timeProblem: " << timeProblem << ", domainProblem: " << domainProblem << std::endl;
...
bool BkgdObsKDLoader::fillBguEntry(
    KD_real *centerLoc,
    int nbrMax,
    float maxDistance,
    ...
    int debug)
{
  ...
  if (debug > 0)
    std::cout << "Point (" << centerLoc[0] << ", " << centerLoc[1] << ", " << centerLoc[2] << ")" << std::endl;
  ...
  for (int inbr = 0; inbr < nbrMax; inbr++)
  {
    ...
    if (debug > 0)
    {
      std::cout << "     bgIn Index: " << bgInIdx / 11 << "  neighbor " << inbr << " (" << bgX << ", " << bgY << ", "
                << exp(logZ) << ") dist: " << sqrt(nbrDistSqs[inbr]) << ", U: " << rhou << ", V: " << rhov << ", W: " << rhow
                << std::endl;
    }
...
bool BkgdObsWRFLoader::loadBkgdObs(std::vector<real> &bgIn)
{
  ...
  if (!dataFile.is_valid())
  {
    std::cout << "Failed to read WRF output netCDF file " << wrffile << std::endl;
    return false;
  }
  ...
  for (int i = 0; i < NLON; i++)
  {
    for (int j = 0; j < NLAT; j++)
    {
      ...
      {
        u[k * NLON * NLAT + j * NLON + i] = (uwind_in[k * (NLON * 1) * NLAT + j * (NLON + 1) + i] +
                                             uwind_in[k * (NLON + 1) * NLAT + j * (NLON + 1) + i + 1]) /
                                            2.0;
        v[k * NLON * NLAT + j * NLON + i] =
            (vwind_in[k * NLON * (NLAT + 1) + j * NLON + i] + vwind_in[k * NLON * (NLAT + 1) + (j + 1) * NLON + i]) / 2.0;
        w[k * NLON * NLAT + j * NLON + i] =
            (wwind_in[k * NLON * NLAT + j * NLON + i] + wwind_in[(k + 1) * NLON * NLAT + j * NLON + i]) / 2.0;
        z[k * NLON * NLAT + j * NLON + i] =
            (ph_in[k * NLON * NLAT + j * NLON + i] + ph_in[(k + 1) * NLON * NLAT + j * NLON + i] +
             phb_in[k * NLON * NLAT + j * NLON + i] + phb_in[(k + 1) * NLON * NLAT + j * NLON + i]) /
            (2.0 * 9.81);
        ...
        double PRESV = (p[k * NLON * NLAT + j * NLON + i] * qv[k * NLON * NLAT + j * NLON + i]) /
                       (0.622 + qv[k * NLON * NLAT + j * NLON + i]);
        double PRESD = p[k * NLON * NLAT + j * NLON + i] - PRESV;
        rhoa[k * NLON * NLAT + j * NLON + i] =
            PRESD / (Rd * t[k * NLON * NLAT + j * NLON + i]) + PRESV / (Rv * t[k * NLON * NLAT + j * NLON + i]);
