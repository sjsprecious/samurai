...
bool CostFunctionXYP::outputAnalysis(const std::string &suffix, real *Astate)
{
  ...
  for (int n = 0; n < analysisSize * analysisDim; n++)
    finalAnalysis[n] = -999.0;
          ...
          for (int jhalf = 0; jhalf <= mishFlag; jhalf++)
          {
            for (int jmu = -jhalf; jmu <= jhalf; jmu++)
            {
              ...
              if (latlonIncr > 0)
              {
                ...
                i = (i - refX) / 1000.0;
                if ((i < iMin) or (i > ((iDim - 1) * DI + iMin)))
                  continue;
                j = (j - refY) / 1000.0;
                if ((j < jMin) or (j > ((jDim - 1) * DJ + jMin)))
                  continue;
              }
              real tpw = 0;
              ...
              for (int pIndex = 1; pIndex < pDim - 1; pIndex++)
              {
                ...
                while ((fabs(peps) > 0.01) and (iter < 5000))
                {
                  ...
                  real kbasis = 0.;
                  real kdbasis = 0.;
                  if (height > ((kDim - 1) * DK + kMin))
                    height = (kDim - 1) * DK + kMin;
                  if (height < (kMin + DK))
                    height = kMin + DK;
                  ...
                  real pdz = (tdz * rhoa + rhoadz * temp) * 287. / 100. +
                ...
                if (((*configHash)["output_mish"] == "false") and (ihalf or jhalf))
                  continue;
                ...
                real satvp = exp(
                    -6096.9385 / temp + 16.635794 - 2.711193e-2 * temp + 1.673952e-5 * temp * temp + 2.433502 * log(temp));
                real vp = temp * rhoq * 461. / 100.;
                ...
                pdx = (tdx * rhoa + rhoadx * temp) * 287. / 100. +
                      (tdx * rhoq + (rhoadx * qv + qvdx * rhoa) * temp / 1000.) * 461. / 100.;
                pdy = (tdy * rhoa + rhoady * temp) * 287. / 100. +
                      (tdy * rhoq + (rhoady * qv + qvdy * rhoa) * temp / 1000.) * 461. / 100.;
                pdz = (tdz * rhoa + rhoadz * temp) * 287. / 100. +
                      (tdz * rhoq + (rhoadz * qv + qvdz * rhoa) * temp / 1000.) * 461. / 100.;
                ...
                if ((*configHash)["output_txt"] == "true")
                {
                  samuraistream << scientific << i << "\t" << j << "\t" << k << "\t" << u << "\t" << v << "\t" << w << "\t"
...
bool CostFunctionXYP::writeNetCDF(const std::string &netcdfFileName)
{
  ...
  if (!(rho = dataFile.add_var("RHOA", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(pheight = dataFile.add_var("Z", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  ...
  if (!(dwdy = dataFile.add_var("DWDY", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(dudz = dataFile.add_var("DUDP", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(dvdz = dataFile.add_var("DVDP", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(dwdz = dataFile.add_var("DWDP", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  ...
  if (!(dpdy = dataFile.add_var("DPDY", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(dtdz = dataFile.add_var("DTDP", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(dqdz = dataFile.add_var("DQVDP", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(dpdz = dataFile.add_var("DZDP", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  ...
  if (!(drhody = dataFile.add_var("DRHODY", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
  if (!(drhodz = dataFile.add_var("DRHODP", nc3Float, timeDim, lvlDim, latDim, lonDim)))
    return NC_ERR;
