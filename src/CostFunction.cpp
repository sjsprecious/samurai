...
bool CostFunction::minimize()
{
  ...
  {
    if (ls_cnt)
      cout << "\t\t (Linesearch iterations = " << ls_cnt << " )" << endl;
  }
...
#pragma acc data copyin(qstate[:nState]) copyout(g[:nState])
    {
      if (its == 0)
        f_val = funcValueAndGradient(qstate, g);
...
#pragma acc parallel loop reduction(+ : grad_dot)
      ...
      cout << "\tNewton Iteration: " << its << "\tJ = " << f_val << "\tResidual = " << n_grad
           << endl;  // prints cost fcn value
...
#pragma acc parallel loop gang vector vector_length(32) private(j)
      ...
      if (verbose)
      {
        std::cout << "\t\tCG iteration " << std::setw(7) << std::right << "0"
                  << ":  r_norm = " << std::fixed << std::setw(20) << std::setprecision(8) << std::right << r0_norm
                  << "     rel_resid = " << std::setw(14) << std::setprecision(10) << std::right << 1.0 << endl;
      }
    }
    // CG LOOP
...
#pragma acc parallel loop reduction(+ : pAp)
      ...
      {
        if (verbose)
          cout << "Negative curvature in CG iteration... pAp = " << pAp << endl;
        neg_curve = 1;
        if (cg_its > 0)
        {         // if first iteration, complete this iteration and then stop
...
#pragma acc parallel loop gang vector vector_length(32) reduction(+ : rr)
      ...
      std::cout << std::right;
      if (verbose)
        cout << "\t\tCG iteration " << std::setw(7) << std::right << cg_its + 1 << ":  r_norm = " << std::setw(20)
             << std::fixed << std::setprecision(8) << std::right << r_norm << "     rel_resid = " << std::setw(14)
             << std::setprecision(10) << std::right << rel_resid << endl;
      ...
      if (neg_curve)
        break;  // this is only at its = 0 => otherwise we already left the loop
...
void CostFunction::conjugateGradient(real *q, real *xi, const real ftol, real fret)
{
  ...
  if (verbose)
    cout << "\tNCG beta type = " << S_BETA_TYPE << endl;
  ...
  {
    if (verbose)
      cout << "\t\tIteration: " << its << "\tJ: " << fq << endl;  // prints cost fcn value
    ...
    if (S_CG_CONV_TYPE != 1)
    {
      ...
      dd = n_grad / n_init_grad;
      if (verbose)
        cout << "\t\t\tRelative norm of gradient = " << dd << endl;
      if (verbose)
        cout << "\t\t\tNorm of gradient = " << n_grad << endl;
    }
...
inline void CostFunction::mov3(real &a, real &b, real &c, const real d, const real e, const real f)
{
...
real CostFunction::dbrent(const real ax, const real bx, const real cx, const real tol, real &xmin)
{
...
void CostFunction::mnbrack(real &ax, real &bx, real &cx, real &fa, real &fb, real &fc)
{
  ...
  while (fb > fc)
  {
    r = (bx - ax) * (fb - fc);
    q = (bx - cx) * (fb - fa);
    u = bx - ((bx - cx) * q - (bx - ax) * r) / (2.0 * CF_SIGN(CF_MAX(fabs(q - r), TINY), q - r));
...
#pragma acc parallel loop private(j)
    ...
    for (i = 0; i < max_funcs; i++)
    {
      ...
      if ((stx != 0) && (((bracket) && (step <= stepmin || step >= stepmax)) ||
                         ((bracket) && (stepmax - stepmin <= rtol * stepmax)) || (infoc == 0)))
      {
        step = stx;
      }
...
#pragma acc parallel loop reduction(+ : dg)
    ...
    if (verbose)
      cout << "\t\tMT LS: Number of function evals = " << n_feval << endl;

    if (verbose)
      cout << "\t\tMT LS: step = " << step << endl;
