              ...
              for (int kIndex = 1; kIndex < kDim - 1; kIndex++)
              {
                for (int khalf = 0; khalf <= mishFlag; khalf++)
                {
                  for (int kmu = -khalf; kmu <= khalf; kmu++)
                  {
                    ...
                    pdr = (tdr * rhoa + rhoadr * temp) * 287. / 100. +
                          (tdr * rhoq + (rhoadr * qv + qvdr * rhoa) * temp / 1000.0) * 461. / 100.;
                    pdt = (tdt * rhoa + rhoadt * temp) * 287. / 100. +
                          (tdt * rhoq + (rhoadt * qv + qvdt * rhoa) * temp / 1000.0) * 461. / 100.;
                    ...
                    if ((*configHash)["output_txt"] == "true")
                    {
                      samuraistream << scientific << i << "\t" << j << "\t" << k << "\t" << u << "\t" << v << "\t" << w
                                    ...
                                    << "\t" << press << "\t" << theta << "\t" << thetae << "\t" << thetaes << "\t" << udr
                                    << "\t" << udt << "\t" << udz << "\t" << vdr << "\t" << vdt << "\t" << vdz << "\t" << wdr
                                    << "\t" << wdt << "\t" << wdz << "\t" << rhowdz * 100. << "\t" << mcresidual << "\t"
                                    << qr << "\n";
                    }
  ...
  if ((*configHash)["output_qc"] == "true")
  {
    ...
    *os++ = "Background";
    ...
    for (int m = 0; m < mObs; m++)
    {
      ...
      datetime obtime;
      obtime = std::chrono::time_point<std::chrono::system_clock>(
          std::chrono::duration<unsigned int>((unsigned int)(unixtime)));  // NCAR - double check
...
bool CostFunctionRTZ::writeNetCDF(const std::string &netcdfFileName)
{
  ...
  if (!(u = dataFile.add_var("U", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(v = dataFile.add_var("V", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(w = dataFile.add_var("W", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(wspd = dataFile.add_var("WSPD", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(relhum = dataFile.add_var("RH", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(hprime = dataFile.add_var("HP", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(qvprime = dataFile.add_var("QVP", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(rhoprime = dataFile.add_var("RHOAP", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(tprime = dataFile.add_var("TP", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(pprime = dataFile.add_var("PP", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(vorticity = dataFile.add_var("VORT", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(divergence = dataFile.add_var("DIV", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(okuboweiss = dataFile.add_var("OW", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(strain = dataFile.add_var("STRAIN", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(tpw = dataFile.add_var("TPW", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(rhou = dataFile.add_var("RHOU", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(rhov = dataFile.add_var("RHOV", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(rhow = dataFile.add_var("RHOW", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(rho = dataFile.add_var("RHOA", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(press = dataFile.add_var("P", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(temp = dataFile.add_var("T", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(qv = dataFile.add_var("QV", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(h = dataFile.add_var("H", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if ((*configHash)["qr_variable"] == "dbz")
  {
    if (!(qr = dataFile.add_var("DBZ", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
      return NC_ERR;
  }
  else
  {
    if (!(qr = dataFile.add_var("QR", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
      return NC_ERR;
  }
  if (!(absVorticity = dataFile.add_var("ABSVORT", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(dewp = dataFile.add_var("DEWPOINT", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(theta = dataFile.add_var("THETA", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(thetae = dataFile.add_var("THETAE", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(thetaes = dataFile.add_var("THETAES", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(dudr = dataFile.add_var("DUDR", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(dvdr = dataFile.add_var("DVDR", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(dwdr = dataFile.add_var("DWDR", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(dudt = dataFile.add_var("DUDT", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(dvdt = dataFile.add_var("DVDT", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(dwdt = dataFile.add_var("DWDT", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(dudz = dataFile.add_var("DUDZ", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(dvdz = dataFile.add_var("DVDZ", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(dwdz = dataFile.add_var("DWDZ", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(dtdr = dataFile.add_var("DTDR", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(dqdr = dataFile.add_var("DQVDR", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(dpdr = dataFile.add_var("DPDR", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(dtdt = dataFile.add_var("DTDT", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(dqdt = dataFile.add_var("DQVDT", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(dpdt = dataFile.add_var("DPDT", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(dtdz = dataFile.add_var("DTDZ", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(dqdz = dataFile.add_var("DQVDZ", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(dpdz = dataFile.add_var("DPDZ", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(drhodr = dataFile.add_var("DRHODR", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(drhodt = dataFile.add_var("DRHODT", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(drhodz = dataFile.add_var("DRHODZ", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
  if (!(mcresidual = dataFile.add_var("MCRESIDUAL", nc3Float, timeDim, lvlDim, thetaDim, radDim)))
    return NC_ERR;
