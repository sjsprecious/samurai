...
bool VarDriver3D::initialize(const XMLNode &configuration)
{
  ...
  if (!parseXMLconfig(configuration))
    return false;
...
bool VarDriver3D::initialize(const samurai_config &configSam)
{
  ...
  if (!parseSamuraiConfig(configSam))
    return false;
...
bool VarDriver3D::validateDriver()
{
  ...
  if (fractlBkgd && (configHash["adjust_background"] == "true"))
    std::cout << "** Warning: 'adjust_background' set to 'true' "
              << "with 'bkgd_obs_interpolation' set to 'fractl' doesn't make sense." << std::endl;

  if (fractlBkgd && !loadBG)
  {
    std::cout << "** Warning: 'bkgd_obs_interpolation' set "
              << "but 'load_background' set to false. Setting load_background to true." << std::endl;
    loadBG = true;
  }
  ...
  if (fractlBkgd)
  {  // Grid comes from the fractl file
  ...
  {  // If runGrid, this will be done in the run(......) call
    ...
    if (loadBG)
    {
      bkgdAdapter = new BkgdStream((dataPath + "/samurai_Background.in")
                                       .c_str());  // Not cross-platform with the '/', but that's fixable later, easily.
    }
    return gridDependentInit();
  }
...
bool VarDriver3D::gridDependentInit()
{
  ...
  for (real k = kmin; k < kmax + kincr; k += kincr)
  {
    ...
    for (int i = 0; i < 6; i++)
    {
      ...
      if (i == 0)
        var = refstate->bhypInvTransform(var);
...
{
  // NOTE (NCAR) : I'm not sure whether we're JUST comparing times, or if dates matter?  Seems not, but get clarification
  // from CSU team
  ...
  for (unsigned int fi = 0; fi < frameVector.size(); fi++)
  {
    ...
    if (Time(reftime) == Time(frametime))
    {
      ...
      cout << "Found matching reference time " << PrintTime(reftime) << " at " << frameVector[fi].getLat() << ", "
           << frameVector[fi].getLon() << "\n";
      return true;
    }
  }
  return false;
    ...
    float *psam)
{
  ...
  if (!validateRunGrid(nx, ny, nsigma, imin, imax, iincr, jmin, jmax, jincr, sigmas))
    return false;
  ...
  if (configHash["array_order"] == "row-major")
    bkgdAdapter =
        new BkgdCArray(nx, ny, nsigma, cdtg, delta, iter, sigmas, latitude, longitude, terrain_Height, u1, v1, w1, th1, p1);
  else
    bkgdAdapter =
        new BkgdFArray(nx, ny, nsigma, cdtg, delta, iter, sigmas, latitude, longitude, terrain_Height, u1, v1, w1, th1, p1);
...
bool VarDriver3D::preProcessMetObs()
{
  ...
  if ((configHash["qr_variable"] == "dbz") and (configHash["load_background"] == "true") and
  ...
  while ((fabs(tmin) > 0.1) and (iter < 5000))
  {
    ...
    real tprime = (refstate->getReferenceVariable(ReferenceVariable::tempref, height + 500.) -
                   refstate->getReferenceVariable(ReferenceVariable::tempref, height - 500.)) /
                  1000.;
    if (tprime != 0)
    {
  ...
  for (std::size_t i = 0; i < filenames.size(); ++i)
  {
    ...
    for (std::size_t i = 0; i < metData->size(); ++i)
    {
      ...
      if ((obTime < startTime) or (obTime >= endTime))
      {
        ...
        //   std::cout << "tcstart: " << PrintDate(startTime_ob) << ", tcend: " << PrintDate(endTime_ob) << ", obTime: " <<
        //   PrintDate(obTime_ob) << std::endl;
        continue;
      }
      ...
      if (runMode == XYZ)
      {
        if ((obX < imin) or (obX > imax) or (obY < jmin) or (obY > jmax) or (obZ < kmin) or (obZ > kmax))
        {
          domainProblem++;
          continue;
        }
      ...
      {
        if ((obRadius < imin) or (obRadius > imax) or (obTheta < jmin) or (obTheta > jmax) or (obZ < kmin) or (obZ > kmax))
        {
          domainProblem++;
          continue;
        }
      ...
      switch (metOb.getObType())
      {
        ...
        case (MetObs::radar):
        {
          ...
          if (ZZ > 0)
          {
            ...
            real Rsquare =
                (iincr * iROI) * (iincr * iROI) + (jincr * jROI) * (jincr * jROI) + (kincr * kROI) * (kincr * kROI);
              ...
              for (int kmu = -1; kmu <= 1; kmu += 2)
              {
                ...
                for (int ii = 0; ii < (idim - 1); ii++)
                {
                  for (int imu = -1; imu <= 1; imu += 2)
                  {
                    ...
                    if (runMode == XYZ)
                    {
                      if (fabs(iPos - obX) > iincr * iROI * 2.)
                        continue;
                    }
                    else if (runMode == RTZ)
                    {
                      if (fabs(iPos - obRadius) > iincr * iROI * 2.)
                        continue;
                    }
                      ...
                      for (int jmu = -1; jmu <= 1; jmu += 2)
                      {
                        ...
                        else if (runMode == RTZ)
                        {
                          ...
                          if (dTheta > jincr * jROI * 2.)
                            continue;
                          rSquare =
                              (obRadius - iPos) * (obRadius - iPos) + (dTheta) * (dTheta) + (obZ - kPos) * (obZ - kPos);
                        }
...
#pragma omp parallel for
    ...
    cout << obVector.size() << " total observations."
         << " ( " << attemptedFiles << " of " << totalFiles << " files processed ) " << endl;
  }
          ...
          for (int jhalf = 0; jhalf <= 1; jhalf++)
          {
            for (int jmu = -jhalf; jmu <= jhalf; jmu++)
            {
              ...
              for (int kIndex = -1; kIndex < kdim; kIndex++)
              {
                for (int khalf = 0; khalf <= 1; khalf++)
                {
                  for (int kmu = -khalf; kmu <= khalf; kmu++)
                  {
                    ...
                    if ((mc_weight > 0.0) and !ihalf and !jhalf and !khalf and (i >= imin) and (i <= imax) and
                        (j >= jmin) and (j <= jmax) and (k >= kmin) and (k <= kmax))
                    {
                      if (runMode == XYZ)
                      {
              ...
              if (!ihalf and !jhalf)
              {
                ...
                if ((maxrefHeight > 0) and (maxrefHeight < kmax) and (pseudow_weight > 0.0))
                {
...
bool VarDriver3D::loadPreProcessMetObs()
{
  ...
  while (obstream >> ob >> error >> iPos >> jPos >> kPos >> type >> time >> wgt[0][0] >> wgt[0][1] >> wgt[0][2] >>
         wgt[0][3] >> wgt[1][0] >> wgt[1][1] >> wgt[1][2] >> wgt[1][3] >> wgt[2][0] >> wgt[2][1] >> wgt[2][2] >> wgt[2][3] >>
         wgt[3][0] >> wgt[3][1] >> wgt[3][2] >> wgt[3][3] >> wgt[4][0] >> wgt[4][1] >> wgt[4][2] >> wgt[4][3] >> wgt[5][0] >>
         wgt[5][1] >> wgt[5][2] >> wgt[5][3] >> wgt[6][0] >> wgt[6][1] >> wgt[6][2] >> wgt[6][3])
  {
    varOb.setOb(ob);
    if (runMode == XYZ)
    {
...
bool VarDriver3D::adjustBackground()
{
  ...
  if (std::stof(configHash["mc_weight"]) > 0)
  {
    for (int iIndex = 0; iIndex < idim; iIndex++)
    {
      ...
      if (i > ((idim - 1) * iincr + imin))
        continue;
      for (int jIndex = 0; jIndex < jdim; jIndex++)
      {
        real j = jmin + jincr * jIndex;
        if (j > ((jdim - 1) * jincr + jmin))
          continue;
        for (int kIndex = 0; kIndex < kdim; kIndex++)
        {
          real k = kmin + kincr * kIndex;
          if (k > ((kdim - 1) * kincr + kmin))
            continue;
          bgObs[p] = 0.0;
...
bool VarDriver3D::validateRunGrid(
    float n_x,
    float n_y,
    float n_z,
    float i_min,
