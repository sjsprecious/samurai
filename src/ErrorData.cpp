...
double *ErrorData::init(std::string fname, HashMap *config, size_t numVar)
{
  ...
  if (dim == NULL)
  {
    std::cout << "Error: ErrorData: " << fname << " doesn't contain Samurai size variables." << std::endl
  ...
  if (success)
  {
    for (size_t src_index = 0, dest_index = 0; src_index < tableDim; src_index += 1, dest_index += varDim)
    {
...
bool ErrorData::writeDebugNc(const std::string &netcdfFileName, bool fMish, double *data)
{
  size_t x, y, z;
  ...
  if (!(v_U = dataFile.add_var("U", nc3Float, timeDim, zDim, yDim, xDim)))
    return NC_ERR;
  if (!(v_V = dataFile.add_var("V", nc3Float, timeDim, zDim, yDim, xDim)))
    return NC_ERR;
  if (!(v_W = dataFile.add_var("W", nc3Float, timeDim, zDim, yDim, xDim)))
    return NC_ERR;
  ...
  for (size_t src_idx = 0, dest_idx = 0; dest_idx < size; src_idx += varDim, dest_idx += 1)
